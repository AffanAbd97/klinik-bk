        ...
        <div class="py-8 px-4 mx-auto ">
            <form action="{{ route('jadwal.store') }}" method="POST">
                ...
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    <div class="sm:col-span-2">
                        ...
                        <select id="hari" name="hari"
                            ...
                            @foreach ($haris as $item)
                                <option @selected($item == old('hari')) value="{{ $item }}">{{ $item }}
                                </option>
                            @endforeach
                        </select>
                        @error('hari')
                    ...
                    <div class="w-full">
                        <label for="jam_mulai"
                            class="block mb-2 text-sm font-medium  {{ $errors->has('jam_mulai') ? 'text-red-600' : 'text-gray-900' }}  dark:text-white">Jam
                            Selesai</label>
                    ...
                    <div class="w-full">
                        <label for="jam_selesai"
                            class="block mb-2 text-sm font-medium  {{ $errors->has('jam_selesai') ? 'text-red-600' : 'text-gray-900' }} dark:text-white">Jam
                            Mulai</label>
                        <input type="time" value="{{ old('jam_selesai') }}" name="jam_selesai" id="jam_selesai"
                    ...
                    <div class="w-full mb-4">
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" value='1' class="sr-only peer" @checked(old('status')=='1') name="status">
                            <div

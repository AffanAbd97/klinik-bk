...
@section('content')
    ...
    <section class="bg-white dark:bg-gray-900">
        <div class="py-8 px-4 mx-auto">
            ...
            <form action="{{ route('dokter.periksa.update', ['periksa' => $periksa]) }}" method="POST">
                @method('put')
                @csrf
                <div class="flex flex-col gap-4">
                    <input type="text" name="idPeriksa" class="hidden" value="{{ $periksa->daftarPoli->id }}">
                    <div class="w-full">
                        <label for="nama" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama
                            Pasien</label>
                        <input type="text" name="nama" id="nama"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                            value="{{ $periksa->daftarPoli->pasien->nama }}" disabled>
                    </div>
                    <div class="w-full">
                    ...
                    <div>
                        ...
                        <select id="obat" multiple name="obat[]"
                            ...
                            @foreach ($periksa->detail as $val)
                                @php
                                    $selectedOptions[] = $val->id_obat;
                                @endphp
                            @endforeach

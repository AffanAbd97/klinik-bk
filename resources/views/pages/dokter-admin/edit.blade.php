...
<section class="md:flex grid grid-cols-1 justify-between items-center mb-8 gap-4">
    ...
    <section class="bg-white dark:bg-gray-900">
        <div class="py-8 px-4 mx-auto ">
            <form action="{{ route('dokter.update',['dokter'=>$dokter]) }}" method="POST">
                @csrf
                @method('put')
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    ...
                    <div class="sm:col-span-2">
                        <label for="no_hp"
                            ...
                            Telepon</label>
                        <input type="text" name="no_hp" id="no_hp" value="{{ old('no_hp') ??$dokter->no_hp }}"
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <select id="Poli" name="poli"
                            ...
                            @forelse($poli as $item)
                                <option value="{{ $item->id }}" @selected(old('poli')==$item->id ||$dokter->id_poli==$item->id )>{{ $item->nama_poli }}</option>
                                @empty
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea id="alamat" rows="4" name="alamat"
                                ...
                                placeholder="Informasi Detail Poli">{{ old('alamat') ??$dokter->alamat  }}</textarea>
                            @error('alamat')
                                <p id="outlined_error_help" class="mt-2 text-xs text-red-600">{{ $message }}</p>
                            @enderror

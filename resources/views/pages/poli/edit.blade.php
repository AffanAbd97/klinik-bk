...
<section class="md:flex grid grid-cols-1 justify-between items-center mb-8 gap-4">
    <div>
        <h1 class="font-semibold text-2xl dark:text-white">Edit Poli</h1>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ms-1 text-lg font-medium text-gray-500 md:ms-2 dark:text-gray-400">Edit Poli</span>
                    </div>
                </li>
            </ol>
        </nav>
    ...
    <section class="bg-white dark:bg-gray-900">
        <div class="py-8 px-4 mx-auto ">
            <form action="{{ route('poli.update',['poli'=>$poli]) }}" method="POST">
                @csrf
                @method('put')
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="nama_poli" id="nama_poli" value="{{ old('nama_poli') ??$poli->nama_poli }}"
                            class="bg-gray-50 border {{ $errors->has('nama_poli') ? 'border-red-600' : 'border-gray-300' }}  text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea id="keterangan" rows="4" name="keterangan"
                            class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border {{ $errors->has('keterangan') ? 'border-red-600' : 'border-gray-300' }} focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            placeholder="Informasi Detail Poli">{{ old('keterangan') ??$poli->keterangan }}</textarea>
                        @error('keterangan')
                            <p id="outlined_error_help" class="mt-2 text-xs text-red-600">{{ $message }}</p>
                        @enderror
